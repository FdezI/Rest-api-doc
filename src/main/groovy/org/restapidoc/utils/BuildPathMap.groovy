...
class BuildPathMap extends AnsiConsoleUrlMappingsRenderer {
    ...
    MappingRules build(def grailsApplication) {
        ...
        def evaluator = Holders.getGrailsApplication().classLoader.loadClass("org.grails.web.mapping.DefaultUrlMappingEvaluator").newInstance(Holders.getGrailsApplication().classLoader.loadClass('org.springframework.mock.web.MockServletContext').newInstance())
        def allMappings = []
